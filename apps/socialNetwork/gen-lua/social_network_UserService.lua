...
function UserServiceClient:ComposeCreatorWithUserId(flow, timestamp, edge, req_id, user_id, username, carrier)
  self:send_ComposeCreatorWithUserId(flow, timestamp, edge, req_id, user_id, username, carrier)
  return self:recv_ComposeCreatorWithUserId(flow, timestamp, edge, req_id, user_id, username, carrier)
end

function UserServiceClient:send_ComposeCreatorWithUserId(flow, timestamp, edge, req_id, user_id, username, carrier)
...
function UserServiceClient:recv_ComposeCreatorWithUserId(flow, timestamp, edge, req_id, user_id, username, carrier)
  local fname, mtype, rseqid = self.iprot:readMessageBegin()
  if mtype == TMessageType.EXCEPTION then
    local x = TApplicationException:new{}
    x:read(self.iprot)
...
function UserServiceProcessor:process_ComposeCreatorWithUserId(seqid, iprot, oprot, server_ctx)
  ...
  iprot:readMessageEnd()
  local result = ComposeCreatorWithUserId_result:new{}
  local status, res = pcall(self.handler.ComposeCreatorWithUserId, self.handler, args.flow, args.timestamp, args.edge, args.req_id, args.user_id, args.username, args.carrier)
  if not status then
    reply_type = TMessageType.EXCEPTION
    result = TApplicationException:new{message = res}
  elseif ttype(res) == 'ServiceException' then
...
return UserServiceClient, UserServiceProcessor