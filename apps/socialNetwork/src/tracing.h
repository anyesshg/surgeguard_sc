...
struct sstats {
  ...
  float exec_full;
  float upscale;
  std::map<int, float> in_time;
  std::map<int, float> exec_sum_time;
};

std::string print_stat(sstats& stat) {
  std::string str = "";
  str += std::to_string(stat.exec_metric) + " ";
  str += std::to_string(stat.exec_full) + " ";
  str += std::to_string(stat.upscale);
  //for(auto it=stat.in_time.begin(); it != stat.in_time.end(); it++) {
  //  str += std::to_string(it->first) + " " + std::to_string(it->second) + " " + std::to_string(stat.exec_sum_time[it->first]);
  //}
  return str;
}
