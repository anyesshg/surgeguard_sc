...
class HomeTimelineHandler : public HomeTimelineServiceIf {
 public:
  ...
  void ReadHomeTimeline(std::vector<Post> &,
...
void HomeTimelineHandler::WriteHomeTimeline(
  ...
  try {
    social_graph_client->GetFollowers(followers_id, flow, timestamp2, level+1, req_id, user_id,
                                      writer_text_map);
  } catch (...) {
...
void HomeTimelineHandler::ReadHomeTimeline(
    ...
    const int8_t flow, const int64_t timestamp2, const int16_t level,
    int64_t req_id, int64_t user_id, int start_idx,
  ...
  try {
    post_client->ReadPosts(_return, flow, timestamp2, level+1, req_id, post_ids, writer_text_map);
  } catch (...) {
