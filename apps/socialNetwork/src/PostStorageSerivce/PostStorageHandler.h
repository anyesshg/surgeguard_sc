...
class PostStorageHandler : public PostStorageServiceIf {
 public:
  ...
  void ReadPost(
    		Post &_return, const int8_t flow, const int64_t timestamp2, const int16_t level,
		int64_t req_id, int64_t post_id,
                const std::map<std::string, std::string> &carrier) override;
 ...
 private:
  ...
  std::string post_storage = "/stats/post-storage";
  std::string post_storage_mc = "/stats/post-storage-mc";
  std::string post_storage_mongo = "/stats/post-storage-mongo";
};
...
PostStorageHandler::PostStorageHandler(
  ...
  //ff = fopen("/social-network-microservices/time_logs/post_storage_log","w");
  cnt.store(100);
}
...
void PostStorageHandler::ReadPosts(std::vector<Post> &_return,
    const int8_t flow, const int64_t timestamp2, const int16_t level,
    int64_t req_id,
    const std::vector<int64_t> &post_ids,
    const std::map<std::string, std::string> &carrier) {
  ...
  //auto tmcs = high_resolution_clock::now();
  memcached_rc =
  ...
  //auto tmce = high_resolution_clock::now();
  //time_str += "MEMCACHE,"+std::to_string(duration<double, std::milli>(tmce-tmcs).count());
  ...
  //auto tmgs = high_resolution_clock::now();
  ...
  if (!post_ids_not_cached.empty()) {
    ...
    //tmcset = high_resolution_clock::now();
  ...
  //auto tmge = high_resolution_clock::now();
  //time_str += ",MONGO,"+std::to_string(duration<double, std::milli>(tmge-tmgs).count());
  ...
  float f1 = duration_cast<microseconds>(t3-t2).count();
  float f2 = duration_cast<microseconds>(t5-t4).count();
  float f3 = duration_cast<microseconds>(t7-t6).count();
  float f4 = duration_cast<microseconds>(t7-t1).count();
 
  float in1 = (ts1.tv_sec*1000000000+ts1.tv_nsec-timestamp2)/1000.0;
  float in2 = duration_cast<microseconds>(t4-t1).count() + in1;
  float in3 = duration_cast<microseconds>(t6-t1).count() + in1;

  stats[0].exec_metric = 0.98*stats[0].exec_metric + 0.02*(f4-f1-f2-f3);
  stats[1].exec_metric = 0.98*stats[1].exec_metric + 0.02*f2;
  stats[2].exec_metric = 0.98*stats[2].exec_metric + 0.02*f3;
    
  stats[0].exec_full = 0.98*stats[0].exec_full + 0.02*(f4-f2-f3); 
  stats[1].exec_full = stats[1].exec_metric;
  stats[2].exec_full = stats[2].exec_metric;

  stats[0].upscale = 0.98*stats[0].upscale + 0.02*((in1-(level))/3);
  stats[1].upscale = 0.98*stats[1].upscale + 0.02*((in2-(level+f4-f1-f2-f3))/4);
  stats[2].upscale = 0.98*stats[2].upscale + 0.02*((in3-(level+f4-f1-f3))/5);


 // float mult = 0.02;
 // if(stats[0].in_time.find(flow) == stats[0].in_time.end()) 
 //   mult = 1;
 // stats[0].in_time[flow] = (1-mult)*stats[0].in_time[flow] + mult*in1;
 // stats[1].in_time[flow+1] = (1-mult)*stats[1].in_time[flow+1] + mult*in2;
 // stats[2].in_time[flow+2] = (1-mult)*stats[2].in_time[flow+2] + mult*in3;
 // 
 // // flow=> edge, level=>sum_exec_metric_prev
 // stats[0].exec_sum_time[flow] = (1-mult)*stats[0].exec_sum_time[flow] + mult*level;
 // stats[1].exec_sum_time[flow+1] = (1-mult)*stats[1].exec_sum_time[flow+1] + mult*(level+f4-f1-f2-f3);
 // stats[2].exec_sum_time[flow+2] = (1-mult)*stats[2].exec_sum_time[flow+2] + mult*(level+f4-f1-f3);
    
  cnt++;
  if(cnt.load() >= 500) {
    mymutex.lock();
    if(cnt.load() >= 500) {
      std::system(("echo " + print_stat(stats[0]) + " > /social-network-microservices/shared/post-storage-service").c_str());
      std::system(("echo " + print_stat(stats[1]) + " > /social-network-microservices/shared/post-storage-memcached").c_str());
      std::system(("echo " + print_stat(stats[2]) + " > /social-network-microservices/shared/post-storage-mongodb").c_str());
      
      //std::string str = std::to_string(exec[0]) + " " + std::to_string(exec2[0]) + " " + std::to_string(in[0]);
      //std::system(("echo " + str + " > /social-network-microservices/shared/post-storage-service").c_str());
  
      //str = std::to_string(exec[1]) + " " + std::to_string(exec2[1]) + " " + std::to_string(in[1]);
      //std::system(("echo " + str + " > /social-network-microservices/shared/post-storage-memcached").c_str());
  
      //str = std::to_string(exec[2]) + " " + std::to_string(exec2[2]) + " " + std::to_string(in[2]);
      //std::system(("echo " + str + " > /social-network-microservices/shared/post-storage-mongodb").c_str());
      cnt.store(0);
    }
    mymutex.unlock();
  }
