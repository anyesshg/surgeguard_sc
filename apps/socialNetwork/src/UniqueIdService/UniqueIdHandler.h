...
class UniqueIdHandler : public UniqueIdServiceIf {
 public:
  ...
  int64_t ComposeUniqueId(int8_t, int64_t, int16_t, 
			  int64_t, PostType::type,
...
int64_t UniqueIdHandler::ComposeUniqueId(
    int8_t flow, int64_t timestamp, int16_t edge,
  ...
  int64_t timestamp2 =
      duration_cast<milliseconds>(system_clock::now().time_since_epoch())
          .count() -
      CUSTOM_EPOCH;
  int idx = GetCounter(timestamp2);
  _thread_lock->unlock();

  std::stringstream sstream;
  sstream << std::hex << timestamp2;
