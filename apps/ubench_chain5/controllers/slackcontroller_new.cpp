...
void read_stats_file() {
  ...
  for(int i=0; i<FLOWS; i++) {
    ...
    flow_okay[i] = true;
    //st_loc_min[i] = 100; st_loc_max[i] = -1;
  }
  
  // Read all the stats files.
  bool queue_issue_found = false;
  bool input_issue_found = false;
...
void upscale_slow(int ctr, int flow, bool downstream_only, bool container_only) {
  if(container_only) {
    upscaled[ctr] = true;
    if(cluster_freqStep[ctr] != MAXSTEP) {
      update_freq(ctr, MAXSTEP);
    } else {
      add_core(ctr);
    }
  }
  ...
  if(downstream_only) {
	...
	break;
      int cluster = downstream[flow][i];
      upscaled[cluster] = true;
...
void downscale(int flow) {
  ...
  for(auto it : container_flow_array[flow]) {
    ...
    if(cluster_freqStep[cluster] > FREQ_STEP_DES) {
      update_freq(cluster, cluster_freqStep[cluster]-2);
      break;
    }
...
void decide() {
  ...
  for(int i=0; i<FLOWS; i++) {
    if(flow_slowdown[i]) {
      ...
      if(st_loc_min[i] != -1) { 
        int edge = node_to_edge(st_loc_min[i], i);
  ...
  for(int iter=0; iter<num_containers; iter++) {
    ...
    if(maxIdx == -1)
      break;
    upscale_slow(maxIdx, -1, false, true);
  }
}
