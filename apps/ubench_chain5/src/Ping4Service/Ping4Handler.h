...
class Ping4Handler : public Ping4ServiceIf {
 public:
  Ping4Handler(ClientPool<ThriftClient<Ping5ServiceClient>> *);
  ~Ping4Handler() override = default;
 ...
 private:
  ClientPool<ThriftClient<Ping5ServiceClient>> * _pong_client_pool;
};

Ping4Handler::Ping4Handler(
    ClientPool<ThriftClient<Ping5ServiceClient>> *pong_client_pool) {
...
void 
Ping4Handler::Ping(int8_t flow, int64_t timestamp, int16_t level, int64_t req_id) {
  ...
  if(cnt.load() >= 400) {
    ...
    // Recheck condition.
      if(cnt.load() >= 400) {
	std::string str = std::to_string(exec_time) + " " + std::to_string(exec2) + " " + std::to_string(in_time[0][4]);
        std::system(("echo " + str + " > /bpf-microservices/shared/ping-service-4").c_str());
        cnt.store(0);
     } 
     mymutex.unlock();
  }
