...
class ClientPool {
 public:
  ...
  TClient * Pop();
  TClient * PopUnlimited();
  void KeepaliveUnlimited(TClient *);
  void Push(TClient *);
...
template<class TClient>
TClient * ClientPool<TClient>::PopUnlimited() {
...
template<class TClient>
void ClientPool<TClient>::KeepaliveUnlimited(TClient *client) {
  int read_pool_size;
  std::unique_lock<std::mutex> cv_lock(_mtx);
  read_pool_size = _curr_pool_size;
  cv_lock.unlock();
  if(read_pool_size > _max_pool_size) 
    Remove(client);
  else
    Push(client);
}
