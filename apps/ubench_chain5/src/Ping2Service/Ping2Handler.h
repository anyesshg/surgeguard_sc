...
class Ping2Handler : public Ping2ServiceIf {
 public:
  Ping2Handler(ClientPool<ThriftClient<Ping3ServiceClient>> *);
  ~Ping2Handler() override = default;
 ...
 private:
  ClientPool<ThriftClient<Ping3ServiceClient>> * _pong_client_pool;
};

Ping2Handler::Ping2Handler(
    ClientPool<ThriftClient<Ping3ServiceClient>> *pong_client_pool) {
...
void 
Ping2Handler::Ping(int8_t flow, int64_t timestamp, int16_t level, int64_t req_id) {
  ...
  if(cnt.load() >= 400) {
    ...
    // Recheck condition.
      if(cnt.load() >= 400) {
	std::string str = std::to_string(exec_time) + " " + std::to_string(exec2) + " " + std::to_string(in_time[0][2]);
        std::system(("echo " + str + " > /bpf-microservices/shared/ping-service-2").c_str());
        cnt.store(0);
     } 
     mymutex.unlock();
  }
