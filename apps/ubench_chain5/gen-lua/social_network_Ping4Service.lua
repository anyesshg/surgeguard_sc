...
Ping4ServiceClient = __TObject.new(__TClient, {
  __type = 'Ping4ServiceClient'
})

function Ping4ServiceClient:Ping(flow, timestamp, level, req_id)
  self:send_Ping(flow, timestamp, level, req_id)
  self:recv_Ping(flow, timestamp, level, req_id)
end

function Ping4ServiceClient:send_Ping(flow, timestamp, level, req_id)
...
function Ping4ServiceClient:recv_Ping(flow, timestamp, level, req_id)
...
Ping4ServiceIface = __TObject:new{
  __type = 'Ping4ServiceIface'
}


Ping4ServiceProcessor = __TObject.new(__TProcessor
, {
 __type = 'Ping4ServiceProcessor'
})

function Ping4ServiceProcessor:process(iprot, oprot, server_ctx)
...
function Ping4ServiceProcessor:process_Ping(seqid, iprot, oprot, server_ctx)
  local args = Ping_args:new{}
...
return Ping4ServiceClient, Ping4ServiceProcessor