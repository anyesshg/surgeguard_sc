...
uint64_t interval_low, interval_high, last_burst_time;
float rep=10, rep_time=0;	// rep + rep_time = 10s.
int burst_length, burst_left;
bool inHigh = false;
int transitions = 0;
uint64_t warmup_time=10000000;
...
int main(int argc, char **argv) {
    ...
    uint64_t throughput2 = (cfg.rate+cfg.burst_req) / cfg.threads;
   
    rep_time = cfg.burst_len/10;
    rep = 10.0 - rep_time; 
    burst_length = (int)((cfg.rate+cfg.burst_req)*rep_time);
    ...
    interval_low = 1000000*connections/throughput;
    interval_high = 1000000*connections/throughput2;
    ...
    for(uint64_t id_url=0; id_url< cfg.num_urls; id_url++ ){
	    ...
	    viol_vol += t->volume/1000.0;

            hdr_add(latency_histogram, t->latency_histogram2);
            hdr_add(real_latency_histogram, t->real_latency_histogram2);
    ...
    printf("Violation volume/req (ms): %f\n", viol_vol/total_complete);
    return 0;
}
...
void *thread_main(void *arg) {
    ...
    hdr_init(1, MAX_LATENCY, 3, &thread->latency_histogram2);
    hdr_init(1, MAX_LATENCY, 3, &thread->real_latency_histogram2);
...
uint64_t gen_exp(connection *c) {
    ...
    uint64_t use_interval = interval_low;
    uint64_t elapsed = time_us() - last_burst_time;

    if(elapsed >= rep*1000000) {
	burst_left--;
	if(burst_left == 0) {
	  transitions++;
	  burst_left = burst_length;
	  last_burst_time = time_us();
        ...
        use_interval = interval_high;
    }

    do {
        z = (double)rand()/RAND_MAX;
    } while ((z == 0) || (z == 1));
    exp_value = (-log(z)*(use_interval));
...
static int response_complete(http_parser *parser) {
	...
	if(now - c->sim_start > warmup_time) {
           hdr_record_value(thread->latency_histogram2, actual_latency_timing);
           hdr_record_value(thread->real_latency_histogram2, actual_latency_timing);
        }

	// For extreme outliers (request latency > 400*QoS_limit, cap the latency to 400*QoS_limit for calculation.
        if(actual_latency_timing > 400*cfg.volume_lim*1000) {
	  thread->volume += (399*cfg.volume_lim*1000);
        } else if(actual_latency_timing > cfg.volume_lim*1000) {
	  thread->volume += (actual_latency_timing - cfg.volume_lim*1000);
        } 
...
static struct option longopts[] = {
    ...
    { "burst_time",      optional_argument, NULL, 'X' },
    { "burst_len",       optional_argument, NULL, 'Y' },
    { "burst_rre",      optional_argument, NULL, 'Z' },
    { "volume_lim",      optional_argument, NULL, 'Q' },
    { NULL,             0,                 NULL,  0  }
};
...
static int parse_args(struct config *cfg, char ***urls, struct http_parser_url **mul_parts, char **headers, int argc, char **argv) {
    ...
    *mul_part = *mul_parts;
    ...
    while ((c = getopt_long(argc, argv, "t:c:s:d:D:H:T:R:X:Y:Z:Q:LPrSpBv?", longopts, NULL)) != -1) {
        switch (c) {
            case 't':
            ...
            case 'c':
                ...
                break;
            case 'X':
		if (scan_metric(optarg, &cfg->burst_time)) return -1;
		break;
	    case 'Y':
		if (scan_metric(optarg, &cfg->burst_len)) return -1;
	  	break; 
	    case 'Z':
		if(scan_metric(optarg, &cfg->burst_req)) return -1;
		break;
	    case 'Q':
		if(scan_metric(optarg, &cfg->volume_lim)) return -1;
		break;
            case 'D':
