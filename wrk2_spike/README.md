# wrk2
  ## Modifications for multiple instance supports
  
  Multiple instance supports are required in many scenarios for more general performance.\
  Multithread concurrent testing on multiple URLs is implemented in the latest version, and
  an overall statistical report will be given.

  ### Usage supplement
  
  It's possible to parse multiple URLs in arguments of the terminal.
 
	wrk -t2 -c100 -d30s -R2000 http://127.0.0.1:8080/index.html http://127.0.0.1:16686
  
  The total number of threads = parameter of ‘-t’ (threads per URL) \* the number of URLs
  
  Description of some existing but unclear features:
  1. -T, --timeout:\
  indicates the duration to determine if a connection is time-out, in other words, a socket is not writable any more.
  2. -s, --script:\
  indicates the Lua script to be loaded.\ 
  If the script is static, namely its "request" is not a function, wrk2 only fetches request messages once per thread during thread initialization.
  Otherwise, request messages will be re-fetched before sending every request.
  3. -D, --dist:\
  indicates the probability distribution of the time interval of sending requests in a connection.\
  "fixed": constant-throughput load, as well as constant request interval\
  "exp": request interval ~ logarithmic distribution\
  "zipf": request interval ~ zipf distribution\
  "norm": sending next requests immediately after last one sent\
  *"exp" and "zipf" make the mean/expectation of request interval the same as the request interval of "fixed".*
  
  Added features:
  1. -S, --separate:\
  When calling multiple instance supports, the default case is to show the overall statistical results on multiple URLs only.\
  The option -S is to respectively show the statistics on each URL.
  2. -r, --requests: \
  The option -r is to print the number of sent requests.
  
  ### Overall statistical assessment
  
  An overal statisticsal result is provided based on the total latency histogram and total throughput histogram, \
  which are generated by accumulating the histogram of each single instance.\
  The overall latency spectrum can be printed out when args '-L'/'--latency' is given.

  ************************************************************************
  ************************************************************************
  Example 1: show the overall statitics only.
  
	wrk -t2 -c100 -d30s -R2000  http://127.0.0.1:16686 http://127.0.0.1:8081
	Running 30s test @ http://127.0.0.1:16686
	  2 threads and 100 connections

	Running 30s test @ http://127.0.0.1:8081
	  2 threads and 100 connections

	  Thread calibration: mean lat.: 35.777ms, rate sampling interval: 181ms
	  Thread calibration: mean lat.: 30.875ms, rate sampling interval: 158ms
	  Thread calibration: mean lat.: 30.947ms, rate sampling interval: 157ms
	  Thread calibration: mean lat.: 35.876ms, rate sampling interval: 182ms

	-----------------------------------------------------------------------
	---------------------------Overall Statistics--------------------------
	-----------------------------------------------------------------------
	  Thread Stats   Avg      Stdev     99%   +/- Stdev
	    Latency    31.43ms   32.06ms 112.51ms   80.17%
	    Req/Sec     1.00k   284.58     1.51k    57.42%
	  Overall 119466 requests in 30.00s, 226.27MB read
	Requests/sec:   3981.70
	Transfer/sec:      7.54MB
  
  Example 2: show respective results on different URLs.

	wrk -t2 -c100 -d30s -R2000 -S  http://127.0.0.1:16686 http://127.0.0.1:8081
	Running 30s test @ http://127.0.0.1:16686
	  2 threads and 100 connections
	...
	Running 30s test @ http://127.0.0.1:8081
	  ...
	  Thread calibration: mean lat.: 32.819ms, rate sampling interval: 176ms
	  Thread calibration: mean lat.: 32.777ms, rate sampling interval: 176ms
	  Thread calibration: mean lat.: 29.338ms, rate sampling interval: 161ms
	  Thread calibration: mean lat.: 29.085ms, rate sampling interval: 160ms

	-----------------------------------------------------------------------
	Test Results @ http://127.0.0.1:16686 
	  Thread Stats   Avg      Stdev     99%   +/- Stdev
	    Latency    36.04ms   36.17ms 128.57ms   80.25%
	    Req/Sec     1.00k   284.90     1.44k    56.64%
	  59785 requests in 30.00s, 180.51MB read
	Requests/sec:   1992.84  
	Transfer/sec:      6.02MB

	-----------------------------------------------------------------------
	Test Results @ http://127.0.0.1:8081 
	  Thread Stats   Avg      Stdev     99%   +/- Stdev
	    Latency    31.03ms   30.90ms 106.88ms   80.17%
	    Req/Sec     1.00k   303.69     1.56k    62.35%
	  59854 requests in 30.00s, 46.01MB read
	Requests/sec:   1994.96  
	Transfer/sec:      1.53MB
	...
	-----------------------------------------------------------------------
	  Thread Stats   Avg      Stdev     99%   +/- Stdev
	    Latency    33.53ms   33.73ms 120.45ms   80.13%
	    Req/Sec     1.00k   294.86     1.52k    59.83%
	  Overall 119639 requests in 30.00s, 226.52MB read
	Requests/sec:   3987.52
	Transfer/sec:      7.55MB
